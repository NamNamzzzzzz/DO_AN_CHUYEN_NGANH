...
void adjust_time(void *pvParameters){
  ...
  while(true){
    ...
    if(current_adjust_state == LOW && last_adjust_state == HIGH){
      ...
      vTaskSuspend(msp_button);
      vTaskSuspend(mrta_button);
      vTaskSuspend(mbv_button);
      vTaskSuspend(mbr_button);
      ...
      if (th_adjust) {
        ...
        vTaskResume(msp_button);
        vTaskResume(mrta_button);
        vTaskResume(mbv_button);
        vTaskResume(mbr_button);
        adjusting = false;
      }
    }
    if (adjusting) {
